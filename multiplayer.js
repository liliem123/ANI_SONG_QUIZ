...
const cfg=window.ANI_FIREBASE_CONFIG||{};
...
function initFirebase(){
  ...
  try{
    ...
    console.log("[ANI MP] Firebase initialized:", cfg.databaseURL);
    $("#firebaseWarning").hidden=true;
    return true;
  }catch(e){
    console.error("[ANI MP] Firebase init failed:",e);
    $("#firebaseWarning").hidden=false;
...
async function createRoom(){
  ...
  for(let i=0;i<10;i++){
    code=randomCode();
    const snap=await get(ref(db,`aniRooms/${code}/meta`));
    if(!snap.exists())break;
  }
  roomCode=code; nickname=name; playerId=randomPlayerId(); isHost=true;

  await set(ref(db,`aniRooms/${roomCode}`),{
    meta:{
...
async function joinRoom(){
  ...
  const roomSnap=await get(ref(db,`aniRooms/${code}/meta`));
  ...
  const pref=ref(db,`aniRooms/${roomCode}/players/${playerId}`);
...
async function addEvent(text,type="system"){
  ...
  const e=push(ref(db,`aniRooms/${roomCode}/events`));
...
async function hostSetPlayback(state, position=null){
  ...
  await set(ref(db,`aniRooms/${roomCode}/meta/playback`),{
    state,
    position:pos,
    updatedAt:Date.now()
  });
...
function applyRemotePlayback(pb){
  ...
  try{
    ...
    console.warn("[ANI MP] playback sync failed",e);
  }finally{
...
function renderAdminWindow(){
  ...
  adminWindow.document.write(`<!doctype html><html lang="ko"><head><meta charset="utf-8"><title>ANI SONG QUIZ ADMIN</title>
  ...
  </head><body><div class="head"><h1>ANI SONG QUIZ · 관리자 정답</h1><span class="live">HOST</span></div>${body}<div class="tip">다음 문제로 이동하면 자동 갱신됩니다.</div></body></html>`);
...
function openAdminWindow(){
  ...
  if(!adminWindow || adminWindow.closed){
    adminWindow=window.open("","ANI_MP_ADMIN","popup=yes,width=430,height=520,resizable=yes,scrollbars=yes");
  }
...
function registerDisconnectCleanup(){
  ...
  const playerRef=ref(db,`aniRooms/${roomCode}/players/${playerId}`);
  const leaveEventRef=push(ref(db,`aniRooms/${roomCode}/events`));
...
function enterRoom(){
  ...
  roomUnsub=onValue(ref(db,`aniRooms/${roomCode}/meta`),snap=>{
  ...
  playersUnsub=onValue(ref(db,`aniRooms/${roomCode}/players`),snap=>{
    playerState=snap.val()||{};
    renderPlayers();
  });

  eventsUnsub=onValue(ref(db,`aniRooms/${roomCode}/events`),snap=>{
...
async function submitAnswer(kind){
  ...
  const answer=kind==="game"?(currentQuestion.anime||currentQuestion.game):currentQuestion.song;
  if(norm(input.value)!==norm(answer))return;

  // 서버 트랜잭션으로 "최초 정답자 1명"만 확정한다.
  const solveRef=ref(db,`aniRooms/${roomCode}/meta/${solvedKey}`);
  ...
  if(tx.committed && tx.snapshot.val()?.playerId===playerId){
    await runTransaction(ref(db,`aniRooms/${roomCode}/players/${playerId}/score`),s=>Number(s||0)+1);
    await addEvent(`${nickname}님이 ${kind==="game"?"애니":"곡"} 정답!  ${answer}`,"correct");
  }
}
...
async function hostReveal(){
  if(!isHost)return;
  await update(ref(db,`aniRooms/${roomCode}/meta`),{forcedReveal:true});
...
async function hostNext(){
  ...
  if(next>=roomState.order.length){
    await update(ref(db,`aniRooms/${roomCode}/meta`),{status:"finished"});
    await addEvent("퀴즈가 종료되었습니다.","system");
    return;
  }
  await update(ref(db,`aniRooms/${roomCode}/meta`),{
...
function isEditableQuizTarget(el){
  if(!el) return false;
  if(el.matches && el.matches('input, textarea, select, [contenteditable="true"]')) return true;
  if(el.closest && el.closest('input, textarea, select, [contenteditable="true"], .autocomplete-wrap')) return true;
  return false;
}
function shouldBlockQuizKey(e){
  if(quizIsRevealed()||isEditableQuizTarget(e.target)) return false;

  // 방 코드/텍스트 선택 후 복사, 붙여넣기, 전체선택, 실행취소 등은 허용
  if((e.ctrlKey||e.metaKey) && ["c","v","x","a","z","y"].includes((e.key||"").toLowerCase())) return false;
...
$("#createRoom").onclick=async()=>{
  ...
  try{
    ...
    console.error("[ANI MP] createRoom failed:",e);
    alert(`방 생성 실패\n${e.message || e}`);
  }finally{
    btn.disabled=false;
    btn.textContent=old;
...
$("#joinRoom").onclick=async()=>{
  ...
  try{
    ...
    console.error("[ANI MP] joinRoom failed:",e);
    alert(`방 참가 실패\n${e.message || e}`);
  }finally{
    btn.disabled=false;
    btn.textContent=old;
...
(async()=>{
  try{
    ...
    await loadQuiz();
    if(!firebaseOK){
      console.warn("[ANI MP] Quiz loaded but Firebase is not ready.");
    }
    console.log("[ANI MP] quiz rows:",quizData.length);
  }catch(e){
    console.error("[ANI MP] startup error:",e);
    $("#firebaseWarning").hidden=false;
